...
*/
    int status = 1;
	double rollPitch;
	int scaled_pitch, scaled_roll;      /* Value of the roll/pitch adjusted to fit 
screen display */
...
display */
    ...
    do
    {
        ...
        /* Switch between roll, pitch, and joystick with the up, down, and right 
button, respectivly */
		if(b_Up == 1){
			status = 1;
		}else if(b_Down == 1){
		...
		if(status == 1){
			...
			rollPitch = roll(x);
			scaled_roll = scaleMagForScreen(rollPitch);
			graph_linetwo(scaled_roll);
		}else if(status == 2){
			//read_input(&t, &x, &y, &z, &b_Up, &b_Down, &b_Left, &b_Right, &j_LX, &j_LY, &j_RX, &j_RY);
			rollPitch = roll(y);
			scaled_pitch = scaleMagForScreen(rollPitch);
